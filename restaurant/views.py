...
class GuestViews(View):
    def guest(self, request, *args, **kwargs):
        ...
        reserved_meals = ReservedMealRepo(
            request=request).list(guest_id=guest.id)
        context['reserved_meals'] = reserved_meals
        reserved_meals_s = json.dumps(
            ReservedMealSerializer(reserved_meals, many=True).data)
...
class MealViews(View):
    ...
    def meal(self, request, *args, **kwargs):
        ...
        reserved_meal = reserved_meal_repo.objects.filter(
            meal=meal).filter(guest=guest).first()
        ...
        if request.user.has_perm(APP_NAME+".view_reservedmeal"):
            served_meals = reserved_meal_repo.list(meal_id=meal.id).exclude(
                date_served=None).order_by('-date_served')
            served_meals_s = json.dumps(
                ReservedMealSerializer(served_meals, many=True).data)
    ...
    def reserved_meal(self, request, *args, **kwargs):
        context = getContext(request=request)
        reserved_meal = ReservedMealRepo(
            request=request).reserved_meal(*args, **kwargs)
        context['reserved_meal'] = reserved_meal
        guest = GuestRepo(request=request).me
        context['guest'] = guest
